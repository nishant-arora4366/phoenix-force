...
-- Add new columns to users table (with IF NOT EXISTS to avoid errors)
ALTER TABLE users 
ADD COLUMN IF NOT EXISTS username VARCHAR(50),
ADD COLUMN IF NOT EXISTS firstname VARCHAR(100) DEFAULT '',
ADD COLUMN IF NOT EXISTS middlename VARCHAR(100),
ADD COLUMN IF NOT EXISTS lastname VARCHAR(100) DEFAULT '',
...
-- Add NOT NULL constraints for required fields (with error handling)
DO $$ 
BEGIN
    -- Add NOT NULL constraint for firstname if not already exists
    BEGIN
    ...
    -- Add NOT NULL constraint for lastname if not already exists
    BEGIN
...
-- Create a view for user profiles with computed fields (optional)
-- This view provides computed fields like full_name and display_name
-- The main users table contains all the actual data
